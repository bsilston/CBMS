...
def extract_title_authors_abstract(lines):
    ...
    for i, line in enumerate(lines):
        lower = line.lower()
        if not found_abstract and "abstract" in lower:
            state = "abstract"
            found_abstract = True
            continue

        if state == "title":
            if re.search(r"\b(and|,|\d)\b", line) and re.search(r"[A-Z][a-z]+\s+[A-Z][a-z]+", line):
                state = "authors"
                author_lines.append(line)
            else:
                title_lines.append(line)
        elif state == "authors":
            if any(kw in lower for kw in ["department", "university", "@", "abstract", "corresponding author", "doi"]):
                continue
            author_lines.append(line)
        elif state == "abstract":
            if re.match(r"^(introduction|keywords|\d+\s)", lower):
                break
